...
export const Conference = () => {
  ...
  if (!expMetrics.isExpMetricsInStorage()) {
    participants.forEach(participant => {
      if (participant.info.name !== undefined && !expMetrics.isParticipantAssociatedWithAnExpMetric(participant.info.name))
        expMetrics.setExpMetric(participant.info.name, 0);
    });
    expMetrics.saveExpMetrics();
  } else {
    expMetrics.loadExpMetrics();
  }

  useEffect(() => {
    const latestParticipant = participants.at(participants.length - 1);
    if (latestParticipant !== undefined 
      && latestParticipant.info.name !== undefined 
    ) {
      expMetrics.setExpMetric(latestParticipant.info.name, 0);
    } 
  }, [participants.length]);
